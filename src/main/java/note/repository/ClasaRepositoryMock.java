...
public class ClasaRepositoryMock implements ClasaRepository{
	...
	{
		int nrmatricolElev = nota.getNrmatricol();
		double punctaj = nota.getNota();
		String materie = nota.getMaterie();
		boolean materieFirstTime = false;

		for(Elev elev : clasa.keySet())
		{
			if (elev.getNrmatricol() == nrmatricolElev)
			{
				if (clasa.get(elev).get(materie) != null)
				{
					clasa.get(elev).get(materie).add(punctaj);
				}
				else
				{
					materieFirstTime = true;
					List<Double> grades = new LinkedList<Double>();

					grades.add(punctaj);
					clasa.get(elev).put(materie, grades);
				}
				break;
			}
		}
		if (materieFirstTime)
		{
			for (Elev elev : clasa.keySet())
			{
				if (elev.getNrmatricol() != nrmatricolElev)
				{
					clasa.get(elev).put(materie, new LinkedList<Double>());
				}
			}
		}
	}
	...
	public List<Corigent> getCorigenti() {
		...
		for(int i = 0; i < corigenti.size() -1; ++i)
		{
			for(int j = i+1; j < corigenti.size(); ++j)
			{
				if (corigenti.get(i).compareTo(corigenti.get(j)) > 0)
				{
					Corigent aux = corigenti.get(i);
					corigenti.set(i, corigenti.get(j));
					corigenti.set(j, aux);
				}
			}
		}
